...
public class DataUsageModule extends ReactContextBaseJavaModule {
    ...
    AsyncTask.execute(new Runnable() {
      ...
      public void run() {
        ...
        try {
          ...
          Date startDate = map.hasKey("startDate") ? new Date(Double.valueOf(map.getDouble("startDate")).longValue())
              : null;
          ...
          if (packageNames != null && packageNames.size() > 0) {
            for (int i = 0; i < packageNames.size(); i++) {
              ...
              try {
                ...
                // JSONObject appStats = getNetworkManagerStats(uid, name, packageName,
                // encodedImage, startDate,
                // endDate);
                // if (appStats != null)
                // apps.put(appStats);
                // }
      ...
      public void run() {
        ...
        try {
          result.put("totalGeral", totalGeral)
              .put("totalGeralMb", String.format("%.2f MB", ((totalGeral / 1024D) / 1024D))).put("apps", apps);
        } catch (JSONException e) {
          Log.e(TAG, "Error parsing JSON: " + e.getMessage(), e);
        }
        ...
        for (PackageInfo packageInfo : packageInfoList) {
          ...
          if (permissions.contains(android.Manifest.permission.INTERNET)) {
            ...
            try {
              ...
              String name = (String) packageManager.getApplicationLabel(appInfo);
              apps.put(new JSONObject().put("name", name).put("packageName", packageName).put("uid", uid));
  ...
  private boolean hasPermissionToReadNetworkHistory(boolean requestPermission) {
    ...
    appOps.startWatchingMode(AppOpsManager.OPSTR_GET_USAGE_STATS, activity.getApplicationContext().getPackageName(),
